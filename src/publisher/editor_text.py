"""마크다운을 서식 없는 평문으로 바꾼다. 핸드오프 페이지의 [평문으로] 버튼이 쓴다.

스마트에디터는 마크다운 문법을 해석하지 않는다. `## 제목`을 그대로 넣으면
화면에도 `## 제목`이라고 남는다. 그래서 문법 기호를 걷어내고
사람이 읽을 수 있는 평문으로 만든다.

기본 경로는 `markdown_html.py`다. 서식이 살아 있는 쪽이 언제나 낫고,
이 파일은 붙여넣은 HTML이 이상하게 앉는 날을 위한 대비책이다.
"""
